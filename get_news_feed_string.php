<?php
    ...
    while ($tour = $DB->NextTour()) {
        ...
        $tour_info .= $tour["tour_id"].','.$tour["user_id"].','.$userName.','.$img.','.$tour["date"].','.$tour["total_time"].','.$tour["total_altitude"].','.$tour["total_distance"].','.$tour["start_lat"].','.$tour["start_lon"].','.$tour["country"].','.$tour["province"].';';
    }
